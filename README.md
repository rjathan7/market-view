...
**The raw material:** all 500 stocks in the S&P 500 (Apple, Nvidia, JPMorgan, etc.). Each one is officially classified into one of ~125 "industries" by GICS (a standard classification system), e.g., NVDA, AMD, and AVGO are all in "Semiconductors"; airlines like Delta and United are in "Passenger Airlines"; banks like JPMorgan and Bank of America are in "Diversified Banks."

**What the app computes daily, for each of those ~125 industries:**

- What % of stocks in that industry are trading above their 50-day average price (a sign of an uptrend)
- What % are near their 52-week high (vs. near their 52-week low)
- How much the industry's stocks went up/down over the last week and the last month
- How volatile/choppy that industry has been recently

It combines those four numbers into a single 0–100 **Health Score** per industry, then sorts all 125 industries from highest to lowest.

**Concrete example:** "Semiconductors" scores 92, meaning most of the ~10 chip stocks (NVDA, AMD, AVGO, TXN, etc.) are near 52-week highs, above their 50-day averages, and trending up. Meanwhile "Solar" scores 31 because most solar stocks (FSLR, ENPH, etc.) are below their averages and down over the last month.

Clicking into "Semiconductors" shows the individual list of those ~10 stocks ranked by recent performance, and a chart showing whether all of them are doing well (broad strength) or just NVDA is carrying the group while the rest lag (concentrated strength).
...
market-view/
│
├── .github/workflows/daily_update.yml   ← the automation (runs everything daily at 6pm ET)
│
├── data/
│   └── market.db                        ← the SQLite database (lives on a separate git branch)
│
├── etl/                                 ← the Python data pipeline
│   ├── backfill.py                      ← one-time script: pulled 2 years of S&P 500 history to seed the DB
│   ├── daily_update.py                  ← runs daily: fetches new prices, computes Health Scores, saves to DB
│   ├── metrics.py                       ← the actual math: breadth, trend, momentum, stability calculations
│   ├── export_json.py                   ← converts the DB results into JSON files the frontend can read
│   ├── db.py                            ← helper for talking to SQLite
│   └── requirements.txt                 ← Python dependencies (pandas, yfinance, etc.)
│
└── frontend/                            ← the React app (deployed on Vercel)
    ├── public/data/
    │   ├── industry_scores.json         ← the computed Health Scores for all ~125 industries
    │   └── stock_metrics.json           ← per-stock data (price, returns, MA status, etc.)
    │
    └── src/
        ├── App.tsx                      ← sets up routing (/ → ranking, /industry/:name → detail)
        ├── types.ts                     ← TypeScript definitions for the data shapes
        ├── hooks/
        │   ├── useIndustryScores.ts     ← fetches industry_scores.json
        │   └── useStockMetrics.ts       ← fetches stock_metrics.json
        ├── lib/
        │   ├── health.ts                ← helpers: score → tier (green/yellow/red), arrows
        │   └── format.ts                ← number formatting helpers
        └── components/
            ├── IndustryRanking.tsx      ← the main screen (ranked list + search)
            ├── IndustryRow.tsx          ← one row in that list (bar, score, arrow)
            ├── IndustryDetail.tsx       ← the drill-down page when you click an industry
            ├── HealthBar.tsx            ← the colored progress bar visualization
            ├── SubScoreBar.tsx          ← breakdown bars (breadth/trend/momentum/stability)
            ├── StockTable.tsx           ← the table of stocks inside an industry
            └── ReturnDistribution.tsx   ← the bar chart showing return distribution
```

The flow in one sentence: `daily_update.py` fetches prices → `metrics.py` computes Health Scores → `export_json.py` writes two JSON files → the React frontend reads those JSON files and renders everything. No live database queries, no backend server, just static JSON files that the browser loads directly.
